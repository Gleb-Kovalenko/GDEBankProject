...
extension DiggerDelegate :URLSessionDataDelegate,URLSessionDelegate {
    public func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        if  DiggerCache.isFileExist(atPath: DiggerCache.cachePath(url: url)){
            ...
            notifyCompletionCallback(DiggerResult.failure(error), diggerSeed)
            return
        }
        /// status code
        ...
        if diggerSeed.progress.fractionCompleted > 1.0 { // file error
            ...
            DiggerCache.moveItem(atPath: diggerSeed.tempPath, toPath: diggerSeed.cachePath)
            notifyCompletionCallback(DiggerResult.success(diggerSeed.cacheFileURL), diggerSeed)
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if let errorInfo = error  {
            ...
            notifyCompletionCallback( DiggerResult.failure(errorInfo), diggerSeed)
...
extension DiggerDelegate {
    ...
    func notifyCompletionCallback(_ result: DiggerResult<URL>,_ diggerSeed: DiggerSeed){
