...
open class DiggerManager: DiggerManagerProtocol {
    ...
    @discardableResult
    public func download(with diggerURL: DiggerURL) throws -> DiggerSeed {
...
extension DiggerManager{
    ...
    func isDiggerURLCorrect(_ diggerURL: DiggerURL) -> DiggerResult<URL> {
        var correctURL: URL
        do {
            correctURL = try diggerURL.asURL()
            return DiggerResult.success(correctURL)
        } catch {
            diggerLog(error)
            return DiggerResult.failure(error)
        }
    }
